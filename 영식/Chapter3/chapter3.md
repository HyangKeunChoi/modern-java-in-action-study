...
자바 API 에서 제공하는 몇몇 함수형 인터페이스에는 람다 표현식을 잘 활용할 수 있는 유틸리티 메서드들이 존재한다. ( 함수형 인터페이스 경우 추상메서드 하나만 존재해야하는데 어떻게 유틸리티 메서드들이 있는가? ⇒ `디폴트 메서드` ( 추상 메서드가 X ) 로 해결 , 디폴트 메서드는 9장에서 좀 더 자세히

## 정리

- 람다식은 익명 함수 일종

  람다식을 왜 사용하는지 ? ⇒ 간결한 코드 구현 , 한명만 사용할 코드를 굳이 클래스나 메서드로 만들지 않고 , 람다식으로 처리하자

- 함수형 인터페이스란 추상메서드 하나만 가지고 있는 인터페이스
  - 람다 표현식은 함수형 인터페이스를 대상으로만 사용 가능하다.
  - 람다 표현식 전체가 함수형 인터페이스의 인스턴스로 취급
- 추상메서드의 형식 ( 함수 디스크립터) 와 람다 표현식의 시그니처가 맞아야지 해당 함수형 인터페이스에 람다식을 사용할 수 있다.
- 함수형 인터페이스 들의 경우 여러 디폴트 메서드를 사용해서 람다식과 같이 다양한 로직을 구현할 수 있다.